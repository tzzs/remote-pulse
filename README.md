...
- **Network**: upload/download rate; optionally plotted as a line in the 30-minute chart on its own right-hand axis, scaled to the window's own peak since it has no natural 0-100% scale like CPU/memory (off by default via `trendChartMetrics`, independent of whether the rate is shown as a detail row)
...
| `remotePulse.trendChartMetrics` | `["cpu", "memory"]` | Which metrics to plot as lines in the trend panel's 30-minute chart — `cpu`, `memory`, `gpu` (primary GPU only), `network` (its own right-hand axis). Independent of `trendPanelSections` and `statusBarMetrics`. Run `Remote Pulse: Configure Trend Chart Metrics` for a real multi-select picker |
